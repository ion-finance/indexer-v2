...
// add 'hash', remove 'eventId'
const swaps = map(rawSwaps, (swap) => {
  const { eventId, ...rest } = swap
  return {
    hash: '',
    ...rest,
  }
...
const deposits = map(rawDeposits, (deposit) => {
  const { eventId, ...rest } = deposit
  return {
    hash: '',
    ...rest,
  }
