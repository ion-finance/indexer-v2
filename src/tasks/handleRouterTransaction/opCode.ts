...
export const normalizeOpcode = (opcode?: string) => {
  if (!opcode) {
    return ''
  }
  if (opcode === 'exception') {
  ...
  // Remove '0x' if present and pad with zeros to ensure it is 8 characters long
  const normalized = opcode.replace(/^0x/, '').padStart(8, '0')
